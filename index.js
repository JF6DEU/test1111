        ...
        switch(urls.pathname){
            ...
            case "/watch":
                if (urls.searchParams.get("v") == null || urls.searchParams.get("v").length >= 50){
                    ...
                    let getresult;
                    try{
                        getresult = execSync(ytdlpPath+" --dump-json https://youtu.be/"+v).toString();
                    } catch(e) {
                        getresult = {};
                    }
                ...
                break;
                case "/geturls":
                    if (urls.searchParams.get("v") == null || urls.searchParams.get("v").length >= 50 || urls.searchParams.get("f") == null || urls.searchParams.get("f").length >= 50){
                        response.writeHead(207, {
                            "Content-Type": "application/json"
                        });
                        message = returnTemplate("./templates/renderjson", {json: "{}"});
                    } else {
                        response.writeHead(200, {
                            "Content-Type": "application/json"
                        });
                        let v = urls.searchParams.get("v").replace(".", "").replace("/", "").replace("&", "").replace("?", "").replace("|", "").replace("(", "").replace(")", "");
                        let f = urls.searchParams.get("f").replace(".", "").replace("/", "").replace("&", "").replace("?", "").replace("|", "").replace("(", "").replace(")", "");
                        let getresult;
                        let outjson;
                        try{
                            getresult = execSync(ytdlpPath+` -f ${f} --dump-json https://youtu.be/`+v).toString();
                        ...
                        message = returnTemplate("./templates/renderjson", {json: JSON.stringify(outjson)});
                    }
                    break;    
            default:
